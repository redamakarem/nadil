            ...
            <div class="owl-carousel owl-theme mb-8">
                @foreach($restaurants as $restaurant)
                    <div class="item flex flex-col justify-center rounded-xl border-2 h-32 shadow-md font-lato bg-blend-darken"
                        ...
                        <a href="{{route('site.restaurants.view',['id'=>$restaurant->id])}}" class="flex flex-col justify-center w-full h-full bg-black opacity-75">
                            <h4 class="text-center font-bold text-white uppercase text-[26px] tracking-[2px] text-opacity-100">{{ $restaurant->{'name_'.app()->getLocale()}  }}</h4>
                            <div
                                class="address text-center text-white uppercase text-[18px] tracking-[2px] text-opacity-100">{{ $restaurant->{'name_'.app()->getLocale()}  }}</div>
                        </a>
                    </div>
                @endforeach
            </div>
